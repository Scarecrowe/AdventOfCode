# 🎄 Advent of Code 2016 - Puzzle Summary

## 📜 Overview

A complete summary of all 2016 Advent of Code puzzles.

Each day includes a short description of the challenge and a link to the full solution implementation.
...
Follow navigation instructions using turns and steps.
Track position on a grid using directional changes.
...
Navigate keypad using directional input.
Build code from resulting button presses.
...
Validate triangles based on side lengths.
Part 1 reads rows as triangles.
...
Validate room names using checksum rules.
Decrypt valid room names using shifting.
...
Generate password using MD5 hashes.
Look for hashes with specific prefixes.
...
Recover message from noisy transmissions.
Select most common character per column.
...
Detect TLS and SSL support in IP strings.
Identify patterns inside and outside brackets.
...
Simulate a small pixel display.
Apply rectangle and rotation commands.
...
Decompress strings with marker instructions.
Part 1 processes markers once.
...
### Day 10 - Balance Bots
Simulate bots passing microchips.
Track comparisons between values.
Part 1 finds specific comparison bot.
...
Move items safely between floors.
Avoid invalid combinations of generators and chips.
...
Interpret simple assembly instructions.
Manipulate registers and jumps.
...
### Day 14 - One-Time Pad
Find keys using stretched MD5 hashing.
...
Align rotating discs to allow capsule drop.
Each disc has position and timing.
...
Generate data using dragon curve expansion.
Compute checksum by pair reduction.
...
Navigate grid using hash-based doors.
Door states depend on current path hash.
...
### Day 18 - Like a Rogue
Generate rows of tiles using rules.
Safe and trap tiles evolve over time.
...
Solve circular elimination problem.
Part 1 uses classic Josephus logic.
...
Determine allowed IP ranges.
Merge and analyse blocked intervals.
...
Scramble string using instruction set.
Includes swaps, rotations, and moves.
...
Analyse storage nodes in grid.
Count viable pairs based on capacity.
...
Extend assembly interpreter with toggle instruction.
Instructions can modify themselves.
...
Grid-based travelling salesman problem.
...
### Day 25 - Clock Signal
Find input producing alternating signal output.
Run assembly program and monitor output.
...
- Each summary is intentionally concise (max 4 lines)
- Focus on problem type, not implementation detail
- Links point directly to your puzzle solutions per day